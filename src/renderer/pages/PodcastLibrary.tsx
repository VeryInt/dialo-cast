...
export default function PodcastLibrary() {
    ...
    useEffect(()=>{
        const getStoredPodcasts = async ()=>{
            ...
            if(dialogueList?.length){
                ...
                setPodcasts(_.map(dialogueList, dialogueItem=>{
    ...
    <div className="container flex flex-col mx-auto max-w-6xl mb-10">
        <AudioPlayer audioFileName={audioFileName} className={`mb-1 relative`} />
                    ...
                    <TableHeader>
                        <TableRow className="border-gray-500 !border-b-2">
                            {_.map(headTextList, (ht, ht_index)=>{
                                return (
                                    <TableHead key={`podcast_table_${ht_index}`} className={ht_index == headTextList.length - 1 ? `flex items-center justify-end pr-6` : ''}>{ht}</TableHead>
                                )
                            })}
                        </TableRow>
                    </TableHeader>
                        ...
                        {_.map(podcasts, (podcast, podcastIndex) => {
                            ...
                            return (
                                <TableRow key={`podcast_${podcastIndex}`} className="border-gray-300">
                                    <TableCell title={title}><div className="flex items-center font-bold line-clamp-1 min-w-36 min-h-10 align-middle">{title}</div></TableCell>
                                    <TableCell>{type}</TableCell>
                                    <TableCell>{date}</TableCell>
                                    <TableCell>
                                        <div className="flex space-x-2 justify-end pr-4 ">
                                            <Play className="w-4 h-4 cursor-pointer" onClick={() => handlePlay(id)} />
                                            <Trash2 className="w-4 h-4 cursor-pointer" onClick={() => handleDelete(id)} />
                                        </div>
                                    </TableCell>
                                </TableRow>
                            )
